...
def get_balance(participant):
    ...
    amount_sent = functools.reduce(lambda tx_sum, tx_amt: tx_sum + tx_amt[0] if len(tx_amt) > 0 else tx_sum,tx_sender, 0)
    # amount_sent = 0
    # for tx in tx_sender: 
    #     if len(tx) > 0:
    #         amount_sent += tx[0]
    ...
    amount_recieved = functools.reduce( lambda tx_sum, tx_amt: tx_sum + tx_amt[0] if len(tx_amt)>0 else tx_sum, tx_recipient, 0)
    # amount_recieved = 0
